(function (root) {
  "use strict";
  const m = (unit, grade, reason, condition) => ({ unit, grade, reason, ...(condition ? { condition } : {}) });
  const matchups = {
    ...
    Marksman: [m("Crawler", "S", "高単発を安価なbodyへ吸わせる"), m("Fang", "A", "screenで射撃を浪費"), m("Wasp", "A", "Air swarm"), m("Rhino", "A", "接近後に射撃を止める", "Rhinoがscreenと射程差を越えて接触できる場合"), m("Stormcaller", "A", "射程外砲撃")],
    ...
    Rhino: [m("Steel Ball", "S", "接近後のbeam duel"), m("Phoenix", "A", "Air高単発"), m("Hacker", "A", "高HPをcontrol"), m("Melting Point", "A", "ramp beam"), m("Sabertooth", "A", "正面single-target", "SabertoothがRhinoを正面で捕捉する場合"), m("Fortress", "A", "巨体frontで突進を受け止める", "FortressがRhinoの進路を塞ぐ場合")],
    ...
    Stormcaller: [m("Rhino", "S", "距離を詰めminimum rangeへ", "Rhinoが砲撃とscreenを抜けてminimum rangeへ入る場合"), m("Phoenix", "A", "Airから処理"), m("Overlord", "A", "Air長射程"), m("War Factory", "A", "迎撃と圧力"), m("Sabertooth", "A", "missile interceptionと前進")],
